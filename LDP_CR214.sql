...
marc_formats AS
	(SELECT 
         ...
         LEFT JOIN folio_derived.instance_ext AS ie ON sm.instance_id = ie.instance_id ::uuid	
    	 WHERE  sm.field = '000'
...
(SELECT DISTINCT
	...
	LEFT JOIN folio_derived.locations_libraries AS ll ON hh.permanent_location_id = ll.location_id  
	LEFT JOIN folio_derived.locations_libraries AS lz ON ite.permanent_location_id = lz.location_id  
   	LEFT JOIN vp25.vs_folio_physical_material_formats AS fmg ON fmg.leader0607 = fm.leader0607
    LEFT JOIN vp25.lm_adc_location_translation_table_9_2023 AS adc2 ON hh.holdings_permanent_location_name=adc2.permanent_location_name)
