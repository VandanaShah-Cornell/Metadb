...
(SELECT DISTINCT sm.instance_id
                ...
                LEFT JOIN folio_derived.instance_ext AS ie ON ie.instance_id::uuid=sm.instance_id
                WHERE 
...
AND trim(concat (he.call_number_prefix,' ',he.call_number,' ',he.call_number_suffix)) NOT ILIKE ALL(ARRAY['on order%', 'in process%', 'Available for the library to purchase', 
 'On selector%', 'vault'])
  ...
  INNER JOIN vp25.vs_folio_physical_material_formats AS mfg ON mf.leader0607=mfg.leader0607
